...
import { fetchChannelVideos, loadCachedVideos, saveCachedVideos } from "@/lib/youtube";
...
export default function VideoLibrary() {
  ...
  /* auto-sync with the YouTube channel (newest first) when an API key is set.
     1) نعرض النسخة المحفوظة في localStorage فورًا (بدون أي انتظار)
     2) الصفحات الجديدة تظهر تدريجيًا أول ما توصل (progressive)
     3) في النهاية نحفظ القائمة الكاملة للزيارة الجاية */
  useEffect(() => {
    let alive = true;
    let shown = 0;
    ...
    fetchChannelVideos((partial) => {
      /* only replace what's on screen if we got MORE videos than we're showing */
      if (alive && partial.length > shown) {
